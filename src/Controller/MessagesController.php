...
class MessagesController extends AppController
{
    ...
    {
        $messages = $this->Messages->find('all', ['conditions'=>['parent_id'=>$parent_id]]);
        echo json_encode([ "status"=>"SUCCESS", "data"=>$messages ]); die();
    }

    public function send()
    {
        $this->autoRender = false;
        $message = $this->Do->adder(['parent_id'=>1, 'message_text'=>'TEST TEST TEST'], 'Messages');
        echo json_encode([ "status"=>"SUCCESS", "data"=>$message ]); die();
    }
